...
describe('NotificationControls', () => {
  ...
  it('presenta el canal listo y el resultado incierto sin prometer entrega', () => {
    render(<NotificationControls companyId="company-1" preference={{
      preference_id: 'preference-1', channel: 'email',
      purpose_code: 'operational_reminder', enabled: true, locale: 'es-CO',
      timezone: 'America/Bogota', quiet_from: '20:00', quiet_until: '07:00',
      updated_at: '2026-09-01T12:00:00Z', destination_state: 'ready',
    }} deliveries={[{
      delivery_id: 'delivery-2', template_code: 'period_due_today',
      context: { period_label: '2026-08 / 2026-08', due_on: '2026-09-05',
        action_url: '/recordatorios?empresa=company-1' },
      status: 'uncertain', suppression_reason: null, attempt_count: 1,
      created_at: '2026-09-04T12:00:00Z', updated_at: '2026-09-04T12:00:00Z',
    }]} />);

    expect(screen.getByText('Canal preparado')).toBeInTheDocument();
    expect(screen.getByText('Resultado por verificar')).toBeInTheDocument();
    expect(screen.getByText(/nunca se reenvía a ciegas/)).toBeInTheDocument();
  });
});
