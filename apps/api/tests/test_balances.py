...
class Cursor:
    ...
    def execute(self, statement: str, params: tuple) -> None:
        if statement.startswith("INSERT INTO fincilia.account_balance"):
            ...
            self.current = None if self.replay else (uuid.uuid4(),)
            return
        if statement.startswith("SELECT b.balance_id"):
            ...
            self.current = (
                ...
                Decimal(self.insert_params[4]), "COP", self.insert_params[6],
                "America/Bogota", 2, 0, "required_pending",
                dt.datetime(2026, 8, 1, tzinfo=dt.timezone.utc),
            )
            return
        if statement.startswith("SELECT observation_key"):
            assert self.insert_params is not None
            self.current = (self.stored_key or self.insert_params[11],)
            return
...
class BalanceTests(unittest.TestCase):
    ...
    def test_server_derives_signed_exact_money_currency_instant_and_lineage(self) -> None:
        cursor = Cursor()
        with patch("fincilia_api.balances._load_evidence", return_value=evidence()):
        ...
        self.assertEqual("required_pending", result["lineage_state"])
        ...
        self.assertEqual(2, cursor.insert_params[8])
        self.assertEqual(0, cursor.insert_params[9])

    def test_exact_replay_returns_same_observation_and_divergence_conflicts(self) -> None:
