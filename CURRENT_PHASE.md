...
| FNC-BET-001 | Beta cerrada sintetica con dominio propio | Review pending | Codex + Integration Steward | Release `90110e7` activo en `fincilia.com`; backup/restore y despliegue in-place verificados; alta→login→empresa→carga→scan/profile/extract pasó en AWS; revisiones independientes siguen pendientes |
