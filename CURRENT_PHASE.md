...
| FNC-SUP-003 | Publicación OIDC de imágenes inmutables al piloto privado | Review pending | Codex + Integration Steward | Run `33829236618` verde sobre `2feb932`: API/web/worker publicados por digest, escaneo sin hallazgos y attestations verificadas; despliegue y revisión independiente pendientes |
| FNC-SUP-004 | Cobertura cerrada de fuentes de imágenes | Review pending | Codex + Integration Steward | Ocho materiales disjuntos, workflow firmante ligado y extractor fail-closed de `COPY`; candidato `33828310759` verificado dentro y fuera del runner; revisión independiente pendiente |
...
| FNC-MAP-001 | Integridad de fuente entre evidencia y mapeo | Review pending | Codex + Integration Steward | V0053/V0054, negativa neutral API, fixtures actuales y CI `33601151681` verde; revisiones Data/Database/Security/Architecture/QA pendientes |
| FNC-MAP-002 | Inmutabilidad de versiones de mapeo | Review pending | Codex + Integration Steward | V0055, maquina de estados, pruebas adversariales y CI `33601151681` verde; revisiones Data/Database/Security/Architecture/QA pendientes |
...
| FNC-GAT-005 | Readiness del piloto privado con datos reales | In progress | Codex + Integration Steward | Candidato firmado `89145a7`, run `33917287024`, fuentes Docker completas, procedencia y SBOM revalidadas fuera del runner; entorno protegido, identidad, restore target y revisiones humanas siguen pendientes |
| FNC-GAT-006 | Evidencia adjudicada de aislamiento, ingreso y canales DRG-01 | Review pending | Codex + Integration Steward | R4 religa el contrato AWS tras añadir alertas brutas; 90 casos y D01-XTENANT/INGRESS/CHANNELS sin cambios; revisión independiente pendiente |
| FNC-GAT-007 | Preflight ejecutable del entorno aislado DRG-00 | Review pending | Codex + Integration Steward | Catálogo único 36/11; consulta live actualizada: foundation 36/36, runtime 0/11, RDS detenido y cero NAT; no mueve gates ni autoriza datos reales |
| FNC-FIN-002 | Sobre de costo verificable para AWS private-pilot | Review pending | Codex + Integration Steward | Plan cold: 142 recursos/41 tipos; piso conocido USD 6,60 no-total, 9 costos cold + 7 warm sin cotizar; CI verde, apply cerrado |
| FNC-FIN-003 | Decisión de costo regional para UAT AWS | Review pending | Codex + Integration Steward | UAT actual USD 31,826/mes y cuenta fija USD 34,258; private-pilot USD 36,20 cold, USD 257,36 warm detenido y USD 319,264 activo; se recomienda endurecer el host existente, sin apply |
| FNC-FIN-004 | Activacion comercial y alertas de costo bruto AWS | Review pending | Codex + Integration Steward | Cuenta PAID/ACTIVE, foundation 36/36 fría y presupuesto bruto USD 120 con alertas 50/80/100 aplicados; plan posterior 147 no-op, runtime y datos reales apagados |
...
| FNC-PLT-012 | Entorno AWS separado del piloto real | In progress | Codex + Integration Steward | Foundation AWS 36/36 aplicada y convergente; runtime, drills, DRG-00/01 y revisión independiente pendientes |
...
| FNC-PLT-015 | Materialización segura de la foundation private-pilot AWS | Review pending | Codex + Integration Steward | Cuenta PAID/ACTIVE, foundation 36/36 y runtime 0/11; RDS detenido con 14 días, plan posterior 147 no-op y datos reales apagados |
| FNC-PLT-016 | Bootstrap seguro de roles y secretos PostgreSQL AWS | In progress | Codex + Integration Steward | Ejecución inicial remota en cero; replay posterior a V0057 denegado y pendiente de diagnóstico/corrección; revisión independiente pendiente |
| FNC-PLT-017 | Ejecucion temporal de bootstrap y migraciones AWS | Review pending | Codex + Integration Steward | Warm temporal, cuatro secretos, bootstrap y 57 migraciones demostrados; 25 recursos efímeros retirados, plan cold 148 no-op y RDS detenido; revisión independiente pendiente |
...
| FNC-UAT-003 | Evidencia verificable del borde HTTPS UAT | Review pending | Codex + Integration Steward | Release `37df2db` activa: TLS 1.3, HSTS, 10 rutas públicas canónicas en inglés y redirects 308 verificados read-only; revisiones pendientes |
...
| FNC-BIL-002 | Stripe Checkout, portal y suscripciones verificables | Review pending | Codex + Integration Steward | Checkout/Portal alojados, webhook firmado serializado, anti-replay, AWS fail-closed y UI completos; apagado sin precios ni credenciales, revisión/aceptación pendientes |
...
| FNC-LEG-002 | Centro legal público Fincilia/Parallext LLC | Review pending | Codex + Integration Steward | Release AWS `37df2db`: centro legal inglés con URLs canónicas inglesas, metadatos canonical y handlers 308 firmados; V0057 conserva historial; revisión independiente pendiente |
| FNC-LEG-003 | Registro verificable de proveedores y subencargados UAT | Review pending | Codex + Integration Steward | AWS/Google/Cloudflare/Namecheap/GitHub separados por plano y dato; fuentes oficiales actuales y divulgación pública verificadas; clasificación, DPA y revisión Legal/Privacy/Security pendientes |
...
| FNC-QA-011 | Aislamiento determinista de la suite PostgreSQL integral | Review pending | Codex + Integration Steward | 431 pruebas PostgreSQL repetibles en volumen persistente; reset vacío V0066 verificado; revisión QA/Database pendiente |
