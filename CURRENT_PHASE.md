...
| FNC-ONB-002 | Registro autoservicio y primer espacio desde la web | In progress | Codex + Integration Steward | Cuenta sintetica, firma y membership owner atomicas; luego alta completa FNC-ONB-001; IdP real permanece pendiente |
| FNC-BET-001 | Beta cerrada sintetica con dominio propio | In progress | Codex + Integration Steward | BETA-01 separado de DRG-01; HTTPS, aislamiento, backups, observabilidad y rollback pendientes de evidencia y revisión |
| FNC-GAT-005 | Readiness del piloto privado con datos reales | In progress | Codex + Integration Steward | 21 controles fail-closed; DRG-00/01 y datos reales continúan bloqueados hasta evidencia y revisión independiente |
| FNC-PLT-004 | Ambiente aislado reproducible para corpus DRG-00 | In progress | Codex + Integration Steward | Laboratorio efímero sin egress, sólo fixtures sintéticos; no autoriza datos reales |
| FNC-DAT-003 | Inventario nominal de artefactos y purga | In progress | Codex + Integration Steward | Ledger append-only, metadata minimizada y reconciliación de zonas en construcción |
| FNC-PRV-003 | Saneamiento, borrado y restore con tombstones | In progress | Codex + Integration Steward | Runbook ejecutable sujeto a L-01 y revisión Legal/Privacy independiente |
| FNC-QA-001 | Ensayo recepción→inventario→purga | In progress | Codex + Integration Steward | Doce escenarios LAB-T01..T12 con datos exclusivamente sintéticos en construcción |
...
| FNC-PLT-013 | Ciclo frío y activación temporal del piloto AWS | Review pending | Codex + Integration Steward | `cold` por defecto, planos reales validados, `pilotctl` fail-closed y estado AWS frío; apply, DRG-00/01 y revisión independiente pendientes |
...
| FNC-IAM-001 | Inicio de sesión Google mediante Cognito | Review pending | Codex + Integration Steward | Backend `3ec2893` y web `99c9445`: Code+PKCE, invitación y alta atómica; configuración AWS, DRG-00 y revisión independiente pendientes |
