...
| FNC-GAT-005 | Readiness del piloto privado con datos reales | In progress | Codex + Integration Steward | Candidato firmado `29f86e8`, run `33828310759`, fuentes Docker completas, procedencia y SBOM revalidadas fuera del runner; entorno protegido, identidad, restore target y revisiones humanas siguen pendientes |
...
| FNC-PLT-016 | Bootstrap seguro de roles y secretos PostgreSQL AWS | In progress | Codex + Integration Steward | Bootstrap SCRAM idempotente, secreto e IAM separados, jobs bootstrap→migrator y controles redactados; apply remoto y revisión independiente pendientes |
| FNC-PLT-017 | Ejecucion temporal de bootstrap y migraciones AWS | In progress | Codex + Integration Steward | Plan warm 26 altas/147 no-op/0 borrados validado; ECS debe permanecer en cero y el cierre vuelve a cold |
