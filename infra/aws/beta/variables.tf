variable "region" {
  ...
  validation {
    ...
    error_message = "FNC-BET-001 solo autoriza sa-east-1."
  }
}
...
variable "release_sha" {
  description = "SHA Git completo que identifica imagenes y bundle."
  type        = string
  validation {
...
variable "beta_domain" {
  description = "FQDN exacto que el operador apuntara al EIP."
  type        = string
  default     = "beta.example.invalid"
  validation {
    condition = length(var.beta_domain) <= 253 && can(regex(
      "^([a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?\\.)+[a-z]{2,63}$", var.beta_domain
    ))
    error_message = "beta_domain debe ser un FQDN DNS en minusculas."
  }
}

variable "gross_monthly_budget_usd" {
  description = "Alerta mensual de la beta; no es un hard cap."
  type        = number
  default     = 35
  validation {
    condition     = var.gross_monthly_budget_usd >= 20 && var.gross_monthly_budget_usd <= 60
    error_message = "El presupuesto beta debe estar entre USD 20 y USD 60."
  }
}
